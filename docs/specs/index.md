...
| [versions-manifest-contract](./versions-manifest-contract.md) | The cross-repo contract for the `https://shll.ai/versions.json` manifest — a static build-time endpoint listing each tool's latest version plus a per-tool notify policy for run-kit's update checker. Output schema (`schema`/`generated_at`/`tools`), keying on the consumer-facing roster name + the `formula`/`envelope` override rules, `v`-strip normalization, the two opposite failure postures (pulled envelopes skip-degrade / site-authored `versions-policy.json` build-stops), the `notify` consumer semantics, the no-workflow-change freshness cascade, and the live-site-swap obligation. The version-and-policy sibling of `help-dump-contract`; machine-anchored to `src/lib/versions-manifest.ts`. |
| [shll-ai-redirect-map-contract](./shll-ai-redirect-map-contract.md) | The cross-repo contract for the `https://hexokit.com/shll-ai-redirects.json` redirect map — a static build-time endpoint enumerating every URL shll.ai ever served and the final hexokit.com page each lands on, for the X2 cutover stub in `sahil87/shll.ai`. Output schema (`schema`/`from`/`to`/`keep`/`redirects`/`rules`), canonical-key normalization, chain collapse with dangling-target build-stop, the derived `rules` ≡ enumeration invariant, the fixture-floor CI checker, and the stub-consumer shape (`meta refresh` + canonical + `location.replace`; `404.html` applying `rules`; `keep` as byte copies). The cutover sibling of `versions-manifest-contract`; machine-anchored to `src/lib/shll-ai-redirects.ts`. |
