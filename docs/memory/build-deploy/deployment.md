...
- **Custom domain**: `cname: hexokit.com`, set via `PUT /repos/sahil87/hexokit-site/pages -f cname=hexokit.com` (send `cname` alone — passing `https_enforced` in the same call fails with "The certificate does not exist yet" until the certificate exists). The `*.github.io` URL 301s to the custom domain once bound.
- **HTTPS**: `https_enforced` can only be set to `true` after GitHub has issued the certificate, which happens asynchronously after the apex resolves to the Pages addresses. Until then the `PUT` returns 404 "The certificate does not exist yet"; retry later. Done state: `curl -sI https://hexokit.com` → 200 with `server: GitHub.com`, `http://` → 301 to https, `www` → 301 to the apex.
...
> **`2lgz` adds `versions.json` as a build output that rides the same cascade.** The `/versions.json` toolkit version manifest (`src/pages/versions.json.ts` → `dist/versions.json`) is a new **build-time static output** joining `dist/`'s emitted files. It reads each `help/<slug>.json` `version` at build time (plus the hand-edited repo-root `versions-policy.json`), so it "rides the daily rebuild" exactly as the `d9qb` star counts and `pgox` one-liners do — a fresh envelope committed by `refresh-help.yml` regenerates the manifest on the next deploy; editing `versions-policy.json` on `main` triggers `deploy.yml` via `on: push`. It adds **no new build dependency** (no third-party API call, no new scheduled pull, no toolchain step; Zod-via-`astro:content` and the envelope readers already exist) and **no workflow change** — the existing refresh→deploy cascade already ships it. Its per-tool skip-degrade posture (a missing/invalid envelope omits that row, build continues) mirrors `CommandReference`'s soft-omit rather than `VersionTable`'s build-stop; the site-authored `versions-policy.json` build-stops. Full derivation in [conventions/versions-manifest](/conventions/versions-manifest.md); cross-repo contract in [`docs/specs/versions-manifest-contract.md`](../../specs/versions-manifest-contract.md).
