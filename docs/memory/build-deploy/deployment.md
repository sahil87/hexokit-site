...
> **Enablement (current state).** Both refresh workflows are **disabled at the repo level** (disabled since the hexokit.com site bootstrap so their old slug map could not scaffold a second `run-kit` tree) and are re-enabled by the operator once `main` carries the `hexokit` slug map: `gh workflow enable refresh-help.yml` / `refresh-readme.yml` (`--repo sahil87/hexokit-site`), then one `workflow_dispatch` seed run each (which also seeds `help/hexokit.json` / `content/hexokit/**` under the new key). Both workflows pin `ref: main`, so enabling before the map lands would run the old map — the enable-then-seed order is load-bearing.
