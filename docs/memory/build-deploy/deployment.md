...
Four steps, all inside the **live** site directory (pinned pnpm 10 + Node 22, the same toolchain as `deploy.yml` and the refresh workflows):
...
4. `node scripts/check-shll-ai-redirects.mjs` (the `Check shll.ai redirect map` step, immediately after `Build site`) — the cross-site shll.ai→hexokit.com redirect map against the committed URL floor: reads `dist/shll-ai-redirects.json` + `scripts/fixtures/shll-ai-paths.txt` and fails the PR when a roster/route change strands an old shll.ai URL or a map value is not a real final page (see [redirect-map](/conventions/redirect-map.md)).
...
> **Enablement (current state).** Both refresh workflows are **active** at the repo level — enabled and seeded on 2026-09-12 (one `workflow_dispatch` run each on `main`, which also seeded `help/hexokit.json` / `content/hexokit/**` under the new key) after sitting disabled since the S1 hexokit.com site bootstrap so their old slug map could not scaffold a second `run-kit` tree (1u4q). Both workflows pin `ref: main`. **Operational fact — same-minute dispatch races on the push to `main`**: dispatching both refresh workflows at once makes the two commit-push steps race; the second push fails `main -> main (fetch first)`. Dispatch them sequentially (or re-run the loser once the winner's commit lands).

### Help refresh

A scheduled job that refreshes `help/<slug>.json` at the repo root (the [help-collection](/conventions/help-collection.md) consume/pull side; forward contract in [`docs/specs/help-dump-contract.md`](../../specs/help-dump-contract.md)). The flow:

1. `.github/workflows/refresh-help.yml` runs on a **daily** `schedule` cron (plus `workflow_dispatch` for on-demand refreshes after a tool release). For each of the 7 tools — a `slug:formula:binary` triple per tool, with the product's triple **`hexokit:run-kit:run-kit`** (the site's slug may differ from the brew formula and binary — the four-name rule, see [tool-roster](/conventions/tool-roster.md)) — it `brew install`s the tool, runs `<binary> help-dump`, captures stdout, and **stamps `captured_at`** (the hexokit.com-owned field the tool does not emit) into the file.
...
> **`pgox` added build-time `help/*.json` consumers, but NO new build dependency category.** The `pgox` per-tool JSON-LD descriptions (`Head.astro`) and the directory-listing one-liners (`ToolsIndex.astro`, rendered on `/toolkit/`) single-source from `help/<slug>.json` `root.short` at build time — so they too "ride the daily rebuild" exactly as the `d9qb` star counts do (a `root.short` edited by `refresh-help.yml` self-corrects on the next deploy). This added two more consumers of the **already-committed** `help/*.json` artifacts (joining `CommandReference`/`VersionTable`/`CommandIndex`); it did **not** introduce a new build dependency — no new third-party API call (unlike the `d9qb` star fetch), no new scheduled pull, no new toolchain step. The read pattern (`repoRootFromModuleUrl` + `HelpDocSchema.parse`) and its freshness model are unchanged. The two `pgox` surfaces split on missing-file posture, as a build property worth noting: `ToolsIndex` **build-stops** on a missing/invalid rostered help file (a committed directory page must be complete — `VersionTable`'s posture), while the inert JSON-LD branch **soft-omits** the description and keeps building (`CommandReference`'s posture) — see [conventions/tool-page-rubric](/conventions/tool-page-rubric.md#tools-index-page--the-tools-directory-landing-change-pgox) and [conventions/seo-social-meta](/conventions/seo-social-meta.md#per-tool-branch--softwareapplication--breadcrumblist-mount-gated).

> **`2lgz` adds `versions.json` as a build output that rides the same cascade.** The `/versions.json` toolkit version manifest (`src/pages/versions.json.ts` → `dist/versions.json`) is a **build-time static output** joining `dist/`'s emitted files and part of the site's **public surface** (a documented cross-repo contract with a live consumer — see the spec's live-site-swap obligation). It reads each `help/<slug>.json` `version` at build time (plus the hand-edited repo-root `versions-policy.json`), so it "rides the daily rebuild" exactly as the `d9qb` star counts and the `ToolsIndex` one-liners do — a fresh envelope committed by `refresh-help.yml` regenerates the manifest on the next deploy; editing `versions-policy.json` on `main` triggers `deploy.yml` via `on: push`. It adds **no new build dependency** (no third-party API call, no new scheduled pull, no toolchain step; Zod-via-`astro:content` and the envelope readers already exist) and **no workflow change** — the existing refresh→deploy cascade already ships it. Its per-tool skip-degrade posture (a missing/invalid envelope omits that row, build continues) mirrors `CommandReference`'s soft-omit rather than `VersionTable`'s build-stop; the site-authored `versions-policy.json` build-stops. The product's row stays keyed **`run-kit`** (the consumer-facing roster name `shll check-updates` matches) while reading `help/hexokit.json` via the policy's `envelope` override. Full derivation in [conventions/versions-manifest](/conventions/versions-manifest.md); cross-repo contract in [`docs/specs/versions-manifest-contract.md`](../../specs/versions-manifest-contract.md).

> **`/shll-ai-redirects.json` joins `versions.json` and the `llms.txt` endpoints as a build-time static output riding the same cascade.** The cross-site shll.ai→hexokit.com redirect map (`src/pages/shll-ai-redirects.json.ts` → `dist/shll-ai-redirects.json`) is a **build-time static endpoint** and part of the site's **public surface** (a documented cross-repo contract with a live consumer — the X2 cutover stub in `sahil87/shll.ai`). It composes the live page set with the in-site redirect table at build time (a dangling in-site target **build-stops**), adds **no new build dependency** and **no workflow change** — the existing refresh→deploy cascade ships it. Full derivation in [conventions/redirect-map](/conventions/redirect-map.md); cross-repo contract in [`docs/specs/shll-ai-redirect-map-contract.md`](../../specs/shll-ai-redirect-map-contract.md).
