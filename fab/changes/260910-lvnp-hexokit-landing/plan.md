...
`#toolkit` SHALL render a heading "The HexoKit toolkit" linking `/toolkit/`, one framing sentence, a decorative inline SVG (`aria-hidden="true"`) of the cube-in-hexagon mark recoloured to `--c-*` tokens, six HTML `<a>` labels positioned around its six edges (`TOOLKIT_EDGES` order, clockwise from the top edge: fab-kit `/fab-kit/`, wt `/wt/`, idea `/idea/`, tu `/tu/`, hop `/hop/`, desktop `/desktop/` — the five tool hrefs derived from `mountFor(slug)`), and an adjacent `<ul>` with one blurb per edge (verbatim from `src/content/docs/toolkit/index.mdx` § The six companions). The framing sentence names `shll` as the installer with a link to `/shll/`. Under 40rem the labels collapse into a plain 2×3 grid beneath the mark.
...
- **GIVEN** the built page
- **WHEN** `/#desktop` is requested
- **THEN** the viewport lands on the card and the id exists exactly once
- **AND** the hexagon's desktop label links `/desktop/` (S3's page), not the anchor
...
#### R13: Starlight config untouched
`astro.config.mjs` MUST NOT change — S3 (`it5d`) already set `starlight.title: 'HexoKit'`, the product description, and the `HeaderNav.astro` nav; the landing inherits all of it through the StarlightPage wrapper.

- **GIVEN** `git diff origin/main...HEAD -- sites/astro-starlight-terminal1/astro.config.mjs`
- **WHEN** inspected
- **THEN** it is empty
...
- No changes to `terminal.css` or `astro.config.mjs` (S3 already set title/description/nav).
...
| 4 | Confident | Version eyebrow reads `help/hexokit.json` through `repoRootFromModuleUrl` with try/catch skip-degrade | Same repo-root idiom as `Head.astro`/`ReadmeSlice`; a missing producer file must never fail the build | S:45 R:95 A:85 D:70 |
